...
it('throws FailedToProcessItemException when item id is missing', function () {
    $itemDataMissingId = [
    ...
    $job = new ProcessItemJob('ITEM_NO_ID', 'token');

    expect(static fn () => $job->handle($mockItemsClient, $mockRepository, $mockLogger))
        ->toThrow(FailedToProcessItemException::class);
});

it('throws FailedToProcessItemException when item title is missing', function () {
    $itemDataMissingTitle = [
        'id' => 'ITEM_NO_TITLE',
        'price' => 99.99,
    ];
    ...
    $mockItemsClient = Mockery::mock(MeliItemsClient::class);
    $mockItemsClient->shouldReceive('getItem')
        ->andReturn($itemDataMissingTitle);
    ...
    $job = new ProcessItemJob('ITEM_NO_TITLE', 'token');

    expect(static fn () => $job->handle($mockItemsClient, $mockRepository, $mockLogger))
        ->toThrow(FailedToProcessItemException::class);
});

it('marks item as failed when validation fails', function () {
    $itemDataInvalid = [
        'id' => '',  // Empty ID
    ...
    $mockRepository->shouldReceive('markAsFailed')
        ->once();
    $mockRepository->shouldNotReceive('saveFromApi');
    $mockRepository->shouldNotReceive('markAsProcessed');

    $job = new ProcessItemJob('ITEM_EMPTY_ID', 'token');
